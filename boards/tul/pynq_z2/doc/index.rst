.. zephyr:board:: pynq_z2

Overview
********

The `TUL PYNQ-Z2`_ is a development board built around the Xilinx Zynq-7000 XC7Z020 All
...
- Two Pmod ports, an Arduino shield connector, a Raspberry Pi header, HDMI in/out and audio codec,
  all attached to the programmable logic

The PYNQ-Z2 uses the same XC7Z020 device and the same MIO assignment as the
:zephyr:board:`pynq_z1`, with the addition of the PS I2C0 controller on ``MIO50``/``MIO51``.
...
The Cadence I2C driver has no pinctrl support, so the ``MIO50``/``MIO51`` multiplexing for I2C0 is
left to the PS initialization performed by the boot loader.
...
Clone and build Das U-Boot for the PYNQ-Z2. Set ``DEVICE_TREE`` to the PYNQ-Z2 device tree shipped
...
.. code-block:: console
   ...
   export DEVICE_TREE="zynq-pynqz2"
   make

If your U-Boot version does not ship a PYNQ-Z2 device tree, use the Xilinx FSBL generated from the
...
microSD card, insert the card in the microSD slot on the PYNQ-Z2 board, ensure the board is
